...
function Test-IndicesHaveUnsupportedMappings
{
    ...
    @{
        "Code" = @("SourceNoDedupeFileContract")
    }
    ...
    foreach ($unsupportedMappingName in $unsupportedMappingNames[$EntityType])
    {
        ...
        $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Get -Command $command -Verbose:$VerbosePreference
...
"@
        ...
        $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Post -Command $command -Body $body -Verbose:$VerbosePreference

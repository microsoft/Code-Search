...
function Restart-Indexing
{
    ...
    $allIndexRecords = (Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Get -Command "_cat/indices?v&h=index&format=json" -Verbose:$VerbosePreference).Content | ConvertFrom-Json
    ...
    else
    {
        foreach ($indexRecord in $allIndexRecords)
        {
            ...
            if ($indexName.StartsWith($EntityType.ToLowerInvariant()))
            {
                ...
                Remove-IndexedDocumentsInBatches -ElasticsearchServiceUrl $ElasticsearchServiceUrl -IndexPattern $indexName -Body "{ `"query`": { `"term`": { `"collectionId`": `"$collectionId`" } } }"
            }
        }
    }

...
function WikiIndexingActivity
{
    ...
    if(IsExtensionInstalled $SQLServerInstance $CollectionDatabaseName "IsCollectionIndexedForWiki")
    {
        ...
        if($bulkIndexingInProgressRepositoryCount -gt 0)
        {
            Write-Host "Count of repositories with indexing in progress: '$bulkIndexingInProgressRepositoryCount'" -ForegroundColor Yellow
        }
        else
        {
            Write-Host "No repositories with indexing in progress" -ForegroundColor Cyan
        }
        ...
        {
            Write-Host "Count of indexing operations failed: '$failedIndexingOperationsCount'" -ForegroundColor Yellow
        }
        else
        {
            Write-Host "No indexing operations failed" -ForegroundColor Cyan
        }
    }
    else
    {

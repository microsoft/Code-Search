...
if($userInput -like "Yes")
{
    ...
    try
    {
        ...
        $ESFolder = Join-Path -Path $tfsFolder.InstallPath -ChildPath Search\ES\elasticsearch-1.7.1-SNAPSHOT\bin
    ...
    if(!$ESFolder -or !(Test-Path -Path $ESFolder))
    {
        ...
        Write-Host "Enter the location to the ES BIN Folder, for e.g. C:\Program Files\Microsoft Team Foundation Server 15.0\Search\ES\elasticsearch-1.7.1-SNAPSHOT\bin : " -ForegroundColor Yellow -NoNewline

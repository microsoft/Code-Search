...
function Request-FixElasticsearchClusterState
{
    ...
    if ($response.StatusCode -eq 200)
    {
        ...
        Write-Log "[MANUAL ACTION REQUIRED] Analyse the cluster allocation explanation respose below and try to mitigate the problem. Sometimes this state can be temporary so execute Repair-Search again. If you are unable to fix the cluster state, contact support.`r`n$clusterAllocationExplanation" -Level Warn
    }
    ...
    {
        Write-Log "It seems like the Elasticsearch cluster just became healthy. You may want to run Repair-Search one more time just to be sure." -Level Warn
    }
    else
    {
        throw "Cluster allocation explain API returned with an unexpected status code [$($response.StatusCode)]."
    }
}
...
"@
        ...
        $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Post -Command $url -Body $body -Verbose:$VerbosePreference
        if ($response.StatusCode -eq 200)
        {
            ...
            if ($impactedDocumentCount -gt 0)
            {
                ...
                {
                    Remove-IndexedDocumentsInBatches -ElasticsearchServiceUrl $ElasticsearchServiceUrl -IndexPattern "$($EntityType.ToLowerInvariant())*" -Body $body

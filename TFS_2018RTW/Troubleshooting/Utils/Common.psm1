...
function Confirm-CollectionIsInStartedState
{
    ...
    if ($status -eq 3)
    {
        throw [ArgumentException]"Collection [$CollectionName] is offline. Please start the collection from Team Foundation Server Administration Console."
    }
...
function Invoke-ElasticsearchCommand
{
    ...
    if ($Method -eq "Head")
    {
        throw [NotSupportedException]"Verb Head is not supported. Use Get instead."
    }
    ...
    try
    {
        ...
        {
            $response = Invoke-WebRequest -Method $Method -Uri $uri -Body $Body -ContentType "application/json" -ErrorAction Stop
        }
...
function Confirm-ElasticsearchIsReachable
{
    ...
    $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Get -Verbose:$VerbosePreference
...
function Remove-IndexedDocumentsInBatches
{
    ...
    do
    {
        $url = "$IndexPattern/_query?timeout=$batchDeleteTimeout"
        $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Delete -Command $url -Body $body
        ...
        $response = Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -Method Post -Command $url
    } while ($timedOut)
}
...
function Test-IndexingFeatureFlagsAreEnabled
{
    ...
    switch ($EntityType)
    {
        "Code"
        {
            if (!(Get-FeatureFlag -SQLServerInstance $SQLServerInstance -ConfigurationDatabaseName $ConfigurationDatabaseName -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.IndexingJobs"))
            {
                Write-Log "Search.Server.IndexingJobs feature flag is not enabled." -Level Error
                return $false
            }
            
            if (!(Get-FeatureFlag -SQLServerInstance $SQLServerInstance -ConfigurationDatabaseName $ConfigurationDatabaseName -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.ContinuousIndexing"))
            {
                Write-Log "Search.Server.ContinuousIndexing feature flag is not enabled." -Level Error
                return $false
            }
        ...
        "WorkItem"
        {
            ...
            elseif (!(Get-FeatureFlag -SQLServerInstance $SQLServerInstance -ConfigurationDatabaseName $ConfigurationDatabaseName -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.WorkItem.ContinuousIndexing"))
            {
                Write-Log "Search.Server.WorkItem.ContinuousIndexing feature flag is not enabled." -Level Error
                return $false
            }
...
function Disable-IndexingFeatureFlags
{
    ...
    switch ($EntityType)
    {
        "Code"
        {
            Set-FeatureFlag -SQLServerInstance $SQLServerInstance -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.IndexingJobs" -State Off
            Set-FeatureFlag -SQLServerInstance $SQLServerInstance -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.ContinuousIndexing" -State Off
            break
        }
        ...
        "WorkItem"
        {
            ...
            Set-FeatureFlag -SQLServerInstance $SQLServerInstance -CollectionDatabaseName $CollectionDatabaseName -CollectionName $CollectionName -FeatureName "Search.Server.WorkItem.ContinuousIndexing" -State Off
            break
        }
...
function Get-ExpectedDocumentContractType
{
    ...
    switch ($EntityType)
    {
        "Code" { return "SourceNoDedupeFileContractV2" }
...
function Write-Log
{
    ...
    Add-Content -Path $LogFilePath $msg -Confirm:$false -WhatIf:$false
}
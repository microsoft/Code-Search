...
{
    # This would not be required after 2018 Update 2 when we make this change in code itself to allowOverride.
    $SqlFullPath = Join-Path $PWD -ChildPath 'SqlScripts\UpdateCodeRepairJobToAllowJobDefinitionOverride.sql'
    Invoke-Sqlcmd -InputFile $SqlFullPath -serverInstance $SQLServerInstance -database $ConfigurationDatabaseName
    Write-Host "Successfully updated job definition override for CodeRepairJob." -ForegroundColor Green

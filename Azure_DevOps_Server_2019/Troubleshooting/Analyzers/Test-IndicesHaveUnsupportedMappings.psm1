...
function Test-IndicesHaveUnsupportedMappings
{
    ...
    $expectedMappingName = Get-ExpectedMappingName -EntityType $EntityType
    if ($unsupportedMappingNames[$EntityType].Contains($expectedMappingName))
    {
        throw "List of unsupported mapping names and/or expected mapping name are incorrectly setup. They both have [$expectedMappingName] in common. Fix the bug in the script."
    }
    ...
    foreach ($unsupportedMappingName in $unsupportedMappingNames[$EntityType])
    {
        ...
        {
            Write-Log "Index [$index] has unsupported mapping. It must be deleted." -Level Error
...
"@
        ...
        $command = "$($indicesWithUnsupportedMapping -join ',')/$unsupportedMappingName/_search?filter_path=aggregations.collections.buckets"
        $aggregationResponse = (Invoke-ElasticsearchCommand -ElasticsearchServiceUrl $ElasticsearchServiceUrl -ElasticsearchServiceCredential $ElasticsearchServiceCredential -Method Post -Command $command -Body $body).Content | ConvertFrom-Json
        foreach ($collection in $aggregationResponse.aggregations.collections.buckets)
        {
            ...
            {
                Write-Log "Collection [$collectionName] has [$($collection.doc_count)] documents indexed with unsupported mapping. It must be re-indexed." -Level Error
                $actionsRecommended += "Restart-Indexing"
            }
            else
            {
                Write-Log "Collection [$collectionName] has [$($collection.doc_count)] documents indexed with unsupported mapping. $EntityType search and indexing might both be failing for this collection. Please execute Repair-Search for this collection separately." -Level Error
            }
        }
    }

    return $actionsRecommended # No actions identified
}